...
class World:  # agent-fluid world
    ...
    def update_distance(self, entity1, entity2):
        ...
        self.distmat[idx1][idx2] = np.sum(self.relposmat[idx1][idx2]**2)
...
#         # distance matrix
#         self.distmat = []
#         # agent name to index map for distance map
#         self.index_map = {}
#         # communication channel dimensionality
#         self.dim_c = 0
#         # position dimensionality
...
#     # calculate distance matrix using matrix
#     def calc_distmat(self):
#         ids = np.arange(len(self.agents))
#         ids_pairs = np.asarray(list(combinations(ids,2)))
#         x = np.asarray([agent.state.p_pos[0] for agent in self.agents])
#         y = np.asarray([agent.state.p_pos[1] for agent in self.agents])
#         dx_pairs = np.asarray(list(combinations(x,2))).ravel()
#         dy_pairs = np.asarray(list(combinations(y,2))).ravel()
#         d_pairs = np.sqrt(dx_pairs**2 + dy_pairs**2)
#         distmat = np.zeros((len(self.agents), len(self.agents)))
#         for i, id in enumerate(ids_pairs):
#             distmat[id[0]][id[1]]=d_pairs[i]
#             distmat[id[1]][id[0]]=d_pairs[i]
#         self.distmat = distmat
#
#     def get_distance(self, agent1, agent2):
#         idx1 = self.index_map[agent1.name]
#         idx2 = self.index_map[agent2.name]
#         return self.distmat[idx1][idx2]
#
#     # update state of the world
#     def step(self):

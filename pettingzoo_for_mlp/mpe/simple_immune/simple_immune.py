...
class raw_env(SimpleEnv, EzPickle):
    def __init__(
        ...
        scenario = Scenario()
        world = scenario.make_world(num_rbc, num_wbc, num_adversaries, num_landmarks)
        SimpleEnv.__init__(
            self,
            scenario=scenario,
            world=world,
        ...
        self.metadata["name"] = "simple_immune"
...
class Scenario(BaseScenario):
    def make_world(self, num_rbc=1, num_wbc=1,num_adversaries=1, num_landmarks=2):
        world = World()
        # set any world properties first
        world.dim_c = 2
        num_good_agents = num_rbc + num_wbc
        num_adversaries = num_adversaries
        ...
        for i, agent in enumerate(world.agents):
            if i < num_rbc:
                ...
                agent.accel = 3.0
                agent.max_speed = 1.0
            elif num_rbc <= i < num_good_agents:
                base_name = "wbc"
                agent.rbc = False
                agent.adversary = False
                base_index = i-num_rbc
                agent.collide = True
                agent.silent = True
                agent.size = 0.075
                agent.accel = 3.0
    ...
    def rbc_reward(self, agent, world):
        # rbc are rewarded for distance to landmarks
        rew = 0
        shape = False
